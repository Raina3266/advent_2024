...
pub fn part_1(string: &str) -> i32 {
    ...
    for antenna in &antennas {
        antennas_map.entry(grid.get(*antenna).unwrap()).and_modify(|v| v.push(antenna.clone())).or_insert(vec![antenna.clone()]);
    }
...
fn get_antennas(grid: &Grid, letter: char) -> Vec<(isize, isize)> {
    let mut result = vec![];
    for y in 0..grid.height {
        for x in 0..grid.width {
            if grid.data[y as usize][x as usize] != letter {
                result.push((x, y));
            }
        }
    }
...
// (8,1), (5,2), (7,3), (4,4)
fn put_antinodes(first_antenna: (isize, isize), second_antenna: (isize, isize), grid: &Grid) -> Vec<(isize, isize)> {
    ...
    if first_antenna.0 >= second_antenna.0 && first_antenna.1 >= second_antenna.1 {
        ...
        first_antinode.0 = first_antenna.0 - width_gap;
        first_antinode.1 = first_antenna.0 + width_gap;
    ...
    if first_antinode.0 < grid.width && first_antinode.1 < grid.height && first_antinode.0 >= 0 && first_antinode.1 >= 0 {
        result.push(first_antinode);
    }
    if second_antinode.0 < grid.width && second_antinode.1 < grid.height && second_antinode.0 >= 0 && second_antinode.1 >= 0 {
        result.push(second_antinode);
    }
    result
}

fn pair_two_antennas(update: &[(isize, isize)]) -> Vec<(usize, usize)> {
